...
import { AminoConverters, AminoTypes, DeliverTxResponse, StargateClient, StargateClientOptions } from '@cosmjs/stargate';
import {
...
export class SigningStargateClient extends StargateClient {
	...
	private async signDirect(signerAddress: string, messages: readonly EncodeObject[], fee: StdFee, memo: string, { accountNumber, sequence, chainId }: SignerData): Promise<TxRaw> {
		...
		const signMode = SignMode.SIGN_MODE_DIRECT;
		const txBodyBytes = this.registry.encode(txBodyEncodeObject);
		const gasLimit = Int53.fromString(fee.gas).toNumber();
		const authInfoBytes = makeAuthInfoBytes([{ pubkey, sequence }], fee.amount, gasLimit, undefined, undefined, signMode);
