...
const CustomSwiper = ({ actions, swiper = true }: SwipersProps) => {
	...
	return (
		<div>
			{swiper ? (
				<Swiper spaceBetween={20} slidesPerView="auto" onSlideChange={s => setActive(s.activeIndex)} centeredSlides className={styles.swiper} initialSlide={0}>
					{actions.map((action, i) => (
						<SwiperSlide className={styles.swiperSlide} key={action.id} onClick={onNavigate}>
							{action.image ? <Image src={`/images/actions/${action.image}`} alt={action.name} layout="fill" className={styles.actionImage} /> : <div className={styles.actionImage} />}
						</SwiperSlide>
					))}
				...
				actions.map((action, i) => (
					<div className={cls(styles.swiperSlide, styles.center)} key={action.id} onClick={onNavigate}>
						{action.image ? (
							<Image src={`/images/actions/${action.image}`} alt={action.name} layout="fill" className={styles.actionImage} placeholder="blur" blurDataURL={shimmerDataUrl()} />
						) : (
