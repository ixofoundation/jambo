...
import { ChevronRightIcon, FileCheckIcon } from '@components/Icons/icons';
...
export default function PiiListScreen() {
  ...
  const [entries, setEntries] = useState<PiiIndexEntry[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const refresh = () => {
          if (cancelled) return;
          setEntries(readAllPiiIndexEntries(mxClient, roomId));
        };

        refresh();

        // Live-refresh the list when a new ixo.pii.index state event lands.
        const room = mxClient.getRoom(roomId);
        const liveState = room?.getLiveTimeline().getState(EventTimeline.FORWARDS);
        if (liveState) {
          const handler = (event: any) => {
            if (event?.getType?.() === 'ixo.pii.index') refresh();
          };
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        {loading && <ListMessage>Loading credential data…</ListMessage>}
        {error && !loading && <ListMessage error>{error}</ListMessage>}
        {!loading && !error && sorted.length === 0 && <ListMessage>No credential data stored yet.</ListMessage>}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {sorted.map((e) => (
              <button
                key={e.cid}
                className='cred-card'
                onClick={() => router.push(`/profile/pii/${encodeURIComponent(e.cid)}`)}
                style={{
                  textAlign: 'left',
                  cursor: 'pointer',
              ...
              >
                <span className='cred-icon' style={{ background: 'var(--purple-tint)', color: 'var(--purple-primary)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 }}>
                  ...
                  >
                    <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{shortenCid(e.cid)}</span>
                    <span aria-hidden>·</span>
                    <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{shortenCid(e.eventId)}</span>
                  </div>
                  <span className='muted' style={{ fontSize: 12.5 }}>
                    Stored at {formatDate(e.storedAt)}
                  </span>
                </div>
