...
export enum STEPS {
	...
	get_delegation_validator_address = 'get_delegation_validator_address',
...
export const steps: { [key in STEPS]: STEP } = {
	...
	[STEPS.get_delegation_validator_address]: {
		id: STEPS.get_delegation_validator_address,
...
export type ReviewStepsTypes = STEPS.bank_MsgSend | STEPS.staking_MsgDelegate | STEPS.staking_MsgUndelegate;
...
interface Get_delegation_validator_address {
	validator: VALIDATOR;
}
...
interface Select_undelegate_amount {
	token: TokenDropdownType;
	amount: number;
}
...
export type AllStepDataTypes =
	...
	| Get_delegation_validator_address
	| Select_token_and_amount
	| Select_delegate_amount
	| Select_undelegate_amount
	| Check_user_balance
	| Define_amount
...
export type StepDataType<T> = T extends STEPS.check_user_balance
	...
	? Get_validator_address
	: T extends STEPS.get_delegation_validator_address
	? Get_delegation_validator_address
